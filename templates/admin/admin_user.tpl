										...
										<div class="form-group">
											<div class="col-sm-12">
												...
												{if $admin->admin_super}
													<button class="btn btn-sm" onclick="document.getElementById('search').value='';document.getElementById('filter_group').value=0;document.getElementById('filter_type').value=3;document.getElementById('filter_status').value=3;document.getElementById('limit').value='50';document.adminForm.p.value=1;"><i class="icon-undo bigger-110"></i> Làm lại</button>
												{else}
													<button class="btn btn-sm" onclick="document.getElementById('search').value='';document.getElementById('filter_group').value=0;document.getElementById('filter_status').value=3;document.getElementById('limit').value='50';document.adminForm.p.value=1;"><i class="icon-undo bigger-110"></i> Làm lại</button>
												{/if}
											</div>
										</div>
									</div>
											...
											{if $admin->admin_super}
												<div class="col-sm-6">
													<select onchange="document.adminForm.submit();" size="1" class="form-control" id="filter_group" name="filter_group">
														<option {if $filter_group==0}selected="selected"{/if} value="0">- Nhóm thành viên -</option>
												...
												<div class="col-sm-6">
													<select onchange="document.adminForm.submit();" size="1" class="form-control" id="filter_type" name="filter_type">
														<option {if $filter_type==3}selected="selected"{/if} value="3">- Kiểu thành viên -</option>
														<option value="0"{if $filter_type == 0} selected{/if}>Admin hệ thống chung</option>
														<option value="1"{if $filter_type == 1} selected{/if}>Admin quản lý dữ liệu trường</option>
														<option value="2"{if $filter_type == 2} selected{/if}>Admin nhà trường</option>
													</select>
												</div>
											{else}
												<div class="col-sm-12">
						...
						<table id="sample-table-1" class="table table-striped table-bordered table-hover">
							...
							{section name=loops loop=$users}
								<tr class="row{if $smarty.section.loops.index%2==0}0{else}1{/if}">
									...
									<td>
										{$users[loops].admin_name}
										{if $users[loops].admin_nickname}
											<br/><i style="color: royalblue;">{$users[loops].admin_nickname}</i>
										{/if}
									</td>
									<td><a href="admin_user.php?task=edit&id={$users[loops].admin_id}" title="Click để sửa thông tin thành viên này">{$users[loops].admin_email}</a></td>
									<td>{$users[loops].admin_username}</td>
									<td align="left"">
										{if $users[loops].admin_type == 1}
											<i style="color:#ff6810;">Admin quản lý dữ liệu trường</i>
										{elseif $users[loops].admin_type == 2}
											<i style="color:#00509b;">Admin nhà trường</i>
										{else}
											<i style="color:rgba(9, 163, 20, 0.67);">Admin hệ thống chung</i>
										{/if}
									</td>
									...
									<td align="left">
										<div class="user_access">
											{foreach from=$users[loops].admin_access key=k item=access}
												...
												{/if}
											{/foreach}
											{if $users[loops].admin_user_group != 1 && $users[loops].admin_access|@count >= 3}
												<div class="readmore">
													<a id="userID{$users[loops].admin_id}" data-sum="{$users[loops].admin_access|@count}" data-uiid="{$users[loops].admin_id}" class="extend" href="#">
														Chi tiết (<b>{$users[loops].admin_access|@count}</b>) <b class="arrow icon-angle-down"></b>
													</a>
												</div>
											{/if}
										</div>
										...
										<div class="widget-body-inner">
											<div class="widget-main">
												...
												<div class="form-group">
													...
													<div class="col-sm-9">
														<input type="text" name="admin_nickname" id="admin_nickname" value="" class="col-xs-12 required" maxlength="150" placeholder="Bút danh của bạn" />
													</div>
												</div>
												<div class="space-4"></div>
												<div class="form-group">
												...
												{if $admin->admin_super}
													<div class="space-4"></div>
													<div class="form-group">
														<label class="col-sm-3 control-label no-padding-right" for="form-field-type"> Admin hệ thống</label>
														<div class="col-sm-9">
															<div class="radio">
																<label>
																	<input type="radio" name="admin_type" value="0" checked="checked" class="ace">
																	<span class="lbl"> Hệ thống chung</span>
																</label>
															</div>
															<div class="radio">
																<label>
																	<input type="radio" name="admin_type" value="1" class="ace">
																	<span class="lbl"> Hệ thống thông tin trường</span>
																</label>
															</div>
															<div class="radio">
																<label>
																	<input type="radio" name="admin_type" value="2" class="ace">
																	<span class="lbl"> Hệ thống quản lý trường</span>
																</label>
															</div>
														</div>
													</div>
										...
										<div class="widget-body-inner">
											<div class="widget-main">
												...
												<div class="form-group">
													<label class="col-sm-3 control-label no-padding-right" for="form-field-nickname"> Bút danh </label>
													<div class="col-sm-9">
														<input type="text" name="admin_nickname" id="admin_nickname" value="{$users.admin_nickname}" class="col-xs-12 required" maxlength="150" placeholder="Bút danh của bạn" />
													</div>
												</div>
												<div class="space-4"></div>
												<div class="form-group">
												...
												{if $admin->admin_super}
													...
													<div class="form-group">
														...
														<div class="col-sm-9">
															<div class="radio">
																<label>
																	<input type="radio" name="admin_type" value="0"{if $users.admin_type == 0} checked="checked"{/if} class="ace">
																	<span class="lbl"> Hệ thống chung</span>
																</label>
															</div>
															<div class="radio">
																<label>
																	<input type="radio" name="admin_type" value="1"{if $users.admin_type == 1} checked="checked"{/if} class="ace">
																	<span class="lbl"> Hệ thống thông tin trường</span>
																</label>
															</div>
															<div class="radio">
																<label>
																	<input type="radio" name="admin_type" value="2"{if $users.admin_type == 2} checked="checked"{/if} class="ace">
																	<span class="lbl"> Hệ thống quản lý trường</span>
																</label>
															</div>
														</div>
...
function submitform(pressbutton){
	...
	if (pressbutton == 'save') {
		if (action == 'add') {
			objUser.processAction("admin_user.php?task=add&ajax=1");
		}
		else if (action == 'edit') {
			objUser.processAction("admin_user.php?task=edit&ajax=1");
		}
	}
	else {
		if (pressbutton) {
...
if (typeof objUser == 'undefined') {
	objUser = {
		onchangeGroup: function() {
			...
			var url = "admin_user.php?task=change_group";
			if (group > 1) {
				$("#rSite").css("display", "");
				$("#rAccess").css("display", "");
