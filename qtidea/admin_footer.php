...
PG_DEBUG&&isset($_benchmark)&&is_object($_benchmark) ? $_benchmark->end('page') : NULL;
PG_DEBUG&&isset($_benchmark)&&is_object($_benchmark) ? $_benchmark->start('shutdown') : NULL;
...
// Add keyword in site
if ( $site_id ){
	require_once ("include/develops/keywords/modules/keyword_in_site.php");
}

if(isset($admin->admin_info['admin_avatar'])&&$admin->admin_info['admin_avatar']){
	$avatar_arr['admin_avatar'] = $admin->admin_info['admin_avatar'];
	$url_crop = ROOT_URL."images/admin/users/crop/". $admin->admin_info['admin_avatar'];
	$url_resize = ROOT_URL."images/admin/users/resize/". $admin->admin_info['admin_avatar'];
	$url_large = ROOT_URL."images/admin/users/large/". $admin->admin_info['admin_avatar'];
	$avatar_arr['admin_avatar_url_crop'] = $url_crop;
	$avatar_arr['admin_avatar_url_resize'] = $url_resize;
	$avatar_arr['admin_avatar_url_large'] = $url_large;
}else{
	$avatar_arr['admin_avatar_url_crop'] = $avatar_arr['admin_avatar_url_resize'] =$avatar_arr['admin_avatar_url_large'] = ROOT_URL."images/admin/users/no_avatar.png";
}
$smarty->assign('avatar_arr', $avatar_arr);
...
// Assign modules install
if (isset($list_modules) && is_array($list_modules) && count($list_modules) && !empty($list_modules)){
	$aryFileModule = array();
	foreach ( $list_modules as $module ){
		if ( !in_array('admin_' . $module['module_code'], $aryFileModule) ){
			array_push($aryFileModule, 'admin_' . $module['module_code']);
		}
	}
	$smarty->assign('list_modules', $list_modules);
	$smarty->assign('aryFileModule', $aryFileModule);
}
if ( $smarty_vertion_3 ){
    $smarty->assignByRef('database', $database);
    $smarty->assignByRef('uri', $uri);
    $smarty->assignByRef('setting', $setting);
    $smarty->assignByRef('admin', $admin);
    $smarty->assignByRef('permission', $permission);
}else{
