...
switch ($task) {
	case "view":
		...
		if ( $admin->admin_super ){
			$filter_type = PGRequest::getInt('filter_type', 3, 'POST');
		}
		...
		if ( !$admin->admin_super ){
			$where[] = " admin_type = " . $admin->admin_info['admin_type'];
		}else{
			if ( $filter_type < 3 ){
				$where[] = " admin_type = " . $filter_type;
			}
		}
		...
		$arySiteId = $admin->get_list_admin_sites();
	...
	case "add":
		...
		if ($ajax) {
			...
			if ( $admin->admin_super )
				$aryInput['admin_type'] = PGRequest::getInt('admin_type', 0, 'POST');
			else
				$aryInput['admin_type'] = $admin->admin_info['admin_type'];
